...
v1.2.11 修复后台在线更新后服务端代码没有自动生效的问题。

### 变更内容

- 后台执行在线更新并成功 `git pull` 后，会自动重启当前 Node.js 服务。
- PM2 环境下会退出当前进程，由 PM2 自动拉起新版进程。
- 直接 `node server.js` 运行时，会关闭当前监听端口并用相同启动参数拉起新进程。
- 后台更新页面会在服务重启后延迟刷新，避免新页面连接到旧 API。

注意：如果当前正在运行的是 `v1.2.10` 或更早版本，旧进程本身还没有自动重启能力。从旧版第一次更新到 `v1.2.11` 后，仍可能需要手动重启一次；之后再通过后台在线更新就会自动重启。
...
v1.2.9 修复 `async` 广告脚本依赖后续容器节点时无法渲染的问题。

### 变更内容

- 带 `async` / `defer` 的外部广告脚本不再阻塞后续广告 DOM 插入。
- 兼容 `script async + div id="container-..."` 形式的广告代码，避免脚本运行时找不到后面的容器节点。
- 普通非 async 广告脚本仍按顺序等待加载，保证 `atOptions` 这类配置脚本先执行。

如果广告平台脚本本身返回 403 或 0 字节，前端无法强制显示广告。请同时确认广告平台后台域名审核、广告位状态和地区投放是否正常。

## 迁移到 v1.2.8

v1.2.8 修复广告联盟脚本加载成功但没有返回广告内容时，前台仍显示空白广告框的问题。

### 变更内容

- 广告脚本执行后会保留短暂等待窗口，兼容外部脚本延迟 `document.write` 写入 iframe 或广告节点。
- 如果广告脚本最终只留下 `<script>`，没有生成 iframe、图片、`ins` 或其他可见广告内容，前台会自动隐藏该广告位。
- 已用 `atOptions` + `highperformanceformat.com/invoke.js` 形式复现：外部脚本请求成功但不返回广告内容时，不再显示 728x90 空白框。

升级后请重启服务，并强制刷新前台页面。广告联盟是否实际返回广告仍取决于广告平台的域名审核、地区、浏览器环境和投放状态。

## 迁移到 v1.2.7

v1.2.7 继续修复第三方广告代码在前台广告位不显示的问题。

### 变更内容

- 前台会执行广告 HTML 任意层级中的 `<script>`，兼容包在 `<div>` 或其他容器里的广告脚本。
- `document.write` / `document.writeln` 写出的后续广告内容会继续按广告代码执行，兼容脚本二次写入脚本或广告节点的情况。
- 后台直接粘贴纯 JS 广告片段时，前台会自动按脚本执行。
- 三个广告位改为串行渲染，避免多个广告位同时接管 `document.write` 时互相影响。

升级后请重启服务，并在浏览器强制刷新前台页面。如果仍显示空白，请重新保存一次对应广告位配置。
...
- 前台会先显示广告容器，再执行广告脚本，避免脚本读取到 0 宽高。
- 前台按代码顺序加载内联脚本和外部脚本，保证 `atOptions` 先于 `invoke.js` 生效。
- 广告脚本执行期间会把 `document.write` / `document.writeln` 临时导向当前广告位，兼容更多广告联盟代码。

升级后请重新保存一次广告位配置，并强制刷新前台页面。

## 迁移到 v1.2.4

v1.2.4 修复部分第三方广告脚本在 `srcdoc` iframe 中加载为空的问题。

### 变更内容

- 前台广告位改为在页面广告容器中按顺序加载 HTML 和脚本，兼容 `atOptions` + 外部 `invoke.js` 这类广告代码。
- 广告位会读取代码里的 `width` / `height`，按广告声明尺寸居中显示，避免出现固定大空白框。
- 后台广告管理说明同步为“前台广告容器中加载”。

升级后请重新保存一次广告位配置，并清理浏览器缓存：
...
- 前台广告位会自动解码广告代码。
- v1.2.4 起，前台广告改为在页面广告容器中按顺序加载，兼容更多第三方广告脚本。
...
- 广告位代码会兼容保存，并在前台广告容器中按顺序加载；页脚 HTML 仍会过滤脚本、事件属性和不安全链接。
