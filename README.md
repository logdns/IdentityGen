...
| 广告位 | Head / AdSense、顶部、信息区下方、地图下方广告代码 | 关闭 |
| 加密币捐赠 | 前台底部捐赠地址列表 | 关闭 |
| 系统版本 | 检查远端版本、执行安全更新 | 当前 Git 版本 |

广告位代码会以兼容格式保存，后台会自动还原显示原始 HTML/JS。Head / AdSense 广告位会由服务端直接注入首页 `<head>`，适合 Google AdSense 站点验证和自动广告代码；其他前台广告位会在广告容器中按顺序加载，并兼容常见 `document.write` 广告脚本。页脚仍只支持有限 HTML，系统会过滤脚本、事件属性和不安全链接；捐赠地址以纯文本显示并支持复制。
...
从 `v1.2.11` 开始，更新成功后服务会自动重启以加载新版 `server.js`，后台页面会在数秒后刷新。若从旧版本升级到 `v1.2.11` 后仍看到旧接口行为，请手动重启一次 Node.js 服务：

```bash
pm2 restart identitygen
```

从 `v1.2.1` 开始，后台认证请求会同时携带会话 token 和兼容密码字段，避免旧 `api.php` 或旧 Node 进程导致“认证失败”。前端 `app.js` / `style.css` 也会使用版本参数并返回 `no-cache`，减少浏览器缓存旧脚本造成的语言或更新按钮异常。
...
npm install -g pm2
pm2 start server.js --name identitygen -- 3002
pm2 save
pm2 startup
```
